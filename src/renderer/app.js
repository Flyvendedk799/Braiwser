...
let toolbar, sidebar, notesPanel, inspectorPanel, aiPanel, auditPanel, tabStrip, webviewHost;
let deviceBadge;
let tabButtons = {};
...
function buildShell() {
  ...
  notesPanel = createNotesPanel(state.config, {
    ...
    onCount: (total) => setTabCount('notes', total),
  });
  ...
  ['notes', 'inspector', 'audit', 'ai'].forEach((id) => {
    const labels = { notes: 'Notes', inspector: 'Inspector', audit: 'Audit', ai: 'AI' };
  ...
  const panel = h('aside', { class: 'panel' }, [tabs, notesPanel.root, inspectorPanel.root, auditPanel.root, aiPanel.root, footer]);
  ...
  const stage = h('div', { class: 'stage' }, [tabStrip.root, webviewHost, deviceBadge, stageOverlay]);
...
function switchTab(id) {
  const panels = { notes: notesPanel, inspector: inspectorPanel, audit: auditPanel, ai: aiPanel };
  if (!panels[id]) return;
  state.activeTab = id;
...
const COMMANDS = {
  ...
  'panel.inspector': () => switchTab('inspector'),
