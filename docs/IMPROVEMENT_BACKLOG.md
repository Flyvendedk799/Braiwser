...
### [x] B9 - Accessibility & quality dedup (CSS + keyboard ops + focus + shared formatters)  `P2` (risk: medium)
...
- [x] B6 UX shortcuts/load-state · [x] B8 goal features (copy-prompt, copy-selector,
  suggest-fix, priority-ordered exports, persist-tabs, **guest console/load-error capture
  surfaced into prompt/markdown exports + agent handoff**, **cross-page locate with ack**)
- [x] B9 (focus-visible, contrast, reduced-motion, aria-labels, modal focus-trap; dedups —
  ai-panel shares esc(), screenshot colors threaded from config, shared format/text.js
  truncate + format/console.js)
- [x] B10 e2e coverage expansion (annotation mutation lifecycle, console capture + export
  surfacing, replay error-path, recording-editor round-trip, reload-restore)

**All 10 batches complete.** e2e grew 51 → **59 checks**, green throughout; clean production
boot. Out of scope by design: guest *network* capture (console-only), and the rejected
scope-creep themes listed above.
